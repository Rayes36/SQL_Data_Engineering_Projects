...
1. [`01_load_flat_data.sql`](./01_load_flat_data.sql) – Load flat CSV into staging table
2. [`02_create_and_load_tables.sql`](./02_create_and_load_tables.sql) – Star schema DDL + dimension, fact, and bridge table population
3. [`03_verify_schema.sql`](./03_verify_schema.sql) – Schema verification queries
...
```text
3_Flat_to_WH_Build/
├── 01_load_flat_data.sql       # Data import from Google Cloud Storage
├── 02_create_and_load_tables.sql # Star schema DDL + dimension, fact, and bridge table population
├── 03_verify_schema.sql        # Schema verification queries
├── build_dw.sql                # Master build script
...
- **Verification:** `03_verify_schema.sql` for record count validation
